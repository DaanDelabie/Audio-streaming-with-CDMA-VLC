...
def main():
    diff_fac = (config_tx['aantal_Users'] * config_tx['difference_Factor']) + config_tx['extra_DC']
    if diff_fac > 9:
        print("ERROR: No matching difference_Factor and extra_DC values. Maximum out going voltage > 9 V")
        print("MAX voltage = (#users * difference_Factor) + extra_DC = ", diff_fac)
    else:
        #########################################################################################
        #                               Audio to bin setup
        #########################################################################################
